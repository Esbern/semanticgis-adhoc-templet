...
- For multi-step analysis scripts, apply the design-before-code gate: write a design brief in `jobs/<job_name>/Analytical_Recipe.md` and get user approval before implementing code.
...
- Follow this recommended lifecycle across all data flows:
	...
	- **design brief for analysis logic** (in `jobs/<job_name>/Analytical_Recipe.md`)
...
- For multi-step analysis scripts, write a design brief in `jobs/<job_name>/Analytical_Recipe.md` and obtain user approval before writing code. Simple single-operation tasks are exempt.
